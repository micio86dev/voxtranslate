...
| 🟠 Med | **Astro 5.18.2 on `website`** — 8 advisories (XSS, Host-header SSRF), fix available. | `website/` | `dashboard` ✅ upgraded to 7.2.9 (production tree now at 0 vulns); `client` was already on 7.1.3. `website` is blocked on a different problem: its `pnpm-workspace.yaml` still holds the literal placeholder `allowBuilds: {esbuild: set this to true or false, sharp: …}`, so `pnpm build` fails before compiling anything and no upgrade can be verified. **Decide the allowBuilds values first** — that is a supply-chain call (`sharp`'s postinstall downloads prebuilt libvips), which is why it was not settled inside a security fix. Then repeat the dashboard's recipe. |
