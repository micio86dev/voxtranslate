...
async function tokenFromServiceAccount(keyPath) {
  ...
  try {
    ...
    fail(
      `Could not read the service account key at ${keyPath}: ${err.message}`,
    );
  }
  if (!key.client_email || !key.private_key) {
    fail(
      `${keyPath} does not look like a service account key (no client_email/private_key).`,
    );
  }
...
async function checkToken(token) {
  ...
  const canEdit = scopes.some(
    (s) =>
      s === SCOPE ||
      s === "https://www.googleapis.com/auth/analytics.manage.users",
  );
  ...
  if (canEdit) {
    console.log(
      `✓ ${SCOPE} present — this token can create custom dimensions.`,
    );
    console.log(
      "  If the Admin API still answers 401/403, the remaining causes are:",
    );
    console.log(
      "  · the Google Analytics Admin API is not enabled on the project that",
    );
    console.log(
      "    issued the token → https://console.cloud.google.com/apis/library/analyticsadmin.googleapis.com",
    );
    console.log(
      "  · the authorising account has no access to the GA4 property.",
    );
  } else {
    console.log(`✗ ${SCOPE} is MISSING from this token.`);
    console.log(
      "  Re-authorise with that exact scope; without it the Admin API refuses",
    );
    console.log(
...
console.log(
  `${parent}: ${existing.length} custom dimensions already registered\n`,
);
...
for (const want of wanted) {
  ...
  if (!found) {
    if (DRY_RUN) {
      console.log(
        `+ would create ${want.parameterName} (${want.scope}) — "${want.displayName}"`,
      );
    } else {
  ...
  if (DRY_RUN) {
    console.log(
      `~ would update ${want.parameterName} (label/description drifted)`,
    );
  } else {
...
if (orphans.length) {
  ...
  console.log(
    "Left alone — archiving one loses its history, so decide that by hand.",
  );
}
if (manual.length) {
  console.log("\nNeeds a manual step:");
