...
describe('subtitleLines', () => {
  it('translated mode shows only the translation', () => {
    expect(
      subtitleLines('translated', { translation: 'Hey everyone', original: 'Ciao', interim: false }),
    ).toEqual([{ cls: 'subtitle-translation', text: 'Hey everyone' }]);
  });

  it('both mode shows translation (prominent) + source line on a final frame', () => {
    expect(
      subtitleLines('both', { translation: 'Hey everyone', original: 'Ciao a tutti', interim: false }),
    ).toEqual([
  ...
  it('original mode shows the source', () => {
    expect(
      subtitleLines('original', { translation: 'Hey everyone', original: 'Ciao', interim: false }),
  ...
  it('translated mode ignores an Enhanced interim that only carries the source', () => {
    ...
    expect(subtitleLines('translated', { original: 'Ciao a', interim: true })).toEqual([]);
  });
});
...
describe('renderSubtitleInto (on-screen)', () => {
  ...
  it("renders the Pro subtitle_final's translation into the DOM (both mode)", () => {
    ...
    const rendered = renderSubtitleInto(area, 'both', {
      translation: "Hey everyone, today we're trying the automatic subtitle translation",
      original: 'Ciao a tutti, oggi proviamo la traduzione automatica dei sottotitoli',
      interim: false,
    });
    expect(rendered).toBe(true);
    const main = area.querySelector('.subtitle-translation');
    expect(main?.textContent).toBe("Hey everyone, today we're trying the automatic subtitle translation");
  ...
  it('renders the translation in translated mode and reports true', () => {
    expect(renderSubtitleInto(area, 'translated', { translation: 'Hey everyone', interim: false })).toBe(true);
  ...
  it('leaves the screen untouched when there is nothing to show (returns false)', () => {
    ...
    expect(renderSubtitleInto(area, 'translated', { original: 'Ciao', interim: true })).toBe(false);
