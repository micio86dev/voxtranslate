...
function webrtcSupported(): boolean {
  ...
  const hasCtor =
    ...
    typeof g.webkitRTCPeerConnection === "function";
...
const VIDEO_BUDGET_MOBILE =
  Number(import.meta.env.PUBLIC_VIDEO_BUDGET_MOBILE) || 1_200_000;
const VIDEO_BUDGET_DESKTOP =
  Number(import.meta.env.PUBLIC_VIDEO_BUDGET_DESKTOP) || 2_400_000;
...
const VIDEO_BUDGET_SCREEN =
  Number(import.meta.env.PUBLIC_VIDEO_BUDGET_SCREEN) || 4_000_000;
...
const sendGame = (state: unknown): void =>
...
function initCallFeatures(m: CallModules): void {
  ...
  tictactoe = new m.tictactoe.TicTacToe(
    minigameEl,
    myId,
    gameName,
    sendGame,
...
const callTimer = new CallTimer({
  ...
  onSet: (cmd) => {
    ...
    if (ttsOn)
      speakSystem(t("timerSetSpeak").replace("{d}", human), getUiLang());
  },
  onDone: () => {
    toast(t("timerDone"));
    playTimerDoneSound();
  ...
  if (!cartesiaManager) {
    cartesiaManager = new callMods!.cartesia.CartesiaManager({
      ...
      translate: (text, source, target) =>
        translateViaServer(text, source, target),
      ...
      onError: (speakerId, status, message) => {
        ...
        console.warn(
          `[cartesia] giving up on ${speakerId} (${status}: ${message})`,
        );
        ws?.send(
          JSON.stringify({ type: "enhanced_fallback", speaker_id: speakerId }),
        );
      },
    });
  }
  ...
  return new Promise((resolve) => {
    ...
    try {
      const mime = ["audio/webm;codecs=opus", "audio/mp4", "audio/webm"].find(
        (ty) => MediaRecorder.isTypeSupported?.(ty),
      );
    ...
    recorder.onstop = () => {
      ...
      resolve(
        speechMs >= MIN_SPEECH_MS && chunks.length
...
const SUBTITLE_CYCLE: SubtitleMode[] = [
  "both",
  "translated",
  "original",
  "off",
...
function renderEngineSelector(): void {
  ...
  for (const e of enginePool()) {
    ...
    if (e.capabilities.cost_scales_per_language) {
      ...
      note.textContent = t(
        auth.isListenerPays() ? "engineCostPerSource" : "engineCostPerLanguage",
      );
      btn.append(note);
    }
...
function enginePool(): EngineInfo[] {
  ...
  if (!auth.isLoggedIn()) {
    ...
    return selectableEngines(
      std.length ? std : availableEngines,
      restrictedNet,
    );
  }
...
function readRecentLangs(): string[] {
  ...
  const fromNav = (navigator.languages ?? []).map((l) =>
...
function pushRecentLang(code: string): void {
  const next = [code, ...readRecentLangs().filter((c) => c !== code)].slice(
    0,
    5,
  );
...
function renderLanguageFirstPicker(): void {
  ...
  if (!offered.has(code)) {
    code =
      readRecentLangs().find((c) => offered.has(c)) ?? [...offered][0] ?? "en";
  }
  wireLangPicker();
...
function wireLangPicker(): void {
  ...
  langSearch.addEventListener("input", () =>
    renderLangPanelList(langSearch.value),
  );
...
function renderLangPanelList(query: string): void {
  ...
  for (const group of languagesByRegion(pool)) {
    ...
    if (group.languages.some((l) => l.code === selectedLang))
      details.open = true;
...
function renderTierNote(tiers: EngineInfo[]): void {
  ...
  if (billing && typeof bal === "number") {
    ...
    balLine.textContent = t("tierBalance").replace(
      "{balance}",
      auth.formatCredits(bal),
    );
...
enterBtn.addEventListener("click", () => {
  ...
  if (visibilityPublic && billing && !auth.isLoggedIn())
    return openSigninGate();
...
$("signin-gate-dismiss").addEventListener("click", () =>
...
function applyPreToggles(): void {
  ...
  const hasLiveVideo =
    !!localStream &&
    localStream.getVideoTracks().some((tr) => tr.readyState === "live");
  ...
  if (!previewOff.hidden) {
    ...
    const avatar =
      billing && auth.isLoggedIn()
        ? auth.avatarUrl(auth.getUser()?.avatar_url, 192)
        : null;
    if (avatar) {
...
async function fetchIceServers(
  ...
  try {
    ...
    const url = restricted
      ? `${HTTP_BASE}/api/ice?restricted=1`
      : `${HTTP_BASE}/api/ice`;
...
async function startCall(): Promise<void> {
  ...
  stageSelfLang.textContent =
    `${FLAG[session.lang] || ""} ${session.lang.toUpperCase()}`.trim();
  show(stageSelf, true);
  ...
  show(
    btnFullscreen,
    !IS_MOBILE && !!document.documentElement.requestFullscreen,
  ); // not needed on mobile

  // Self cell — reflect the pre-join mic/camera choice.
  const myAvatar =
    billing && auth.isLoggedIn() ? auth.getUser()?.avatar_url : null;
  addCell(
    myId,
    session.name || t("namePlaceholder"),
    session.lang,
    true,
    myAvatar,
  );
  ...
  if (session?.engine && restrictedNet) {
    const safe = enforceEngineForNetwork(
      session.engine,
      availableEngines,
      true,
    );
...
function openSocket(): void {
  ...
  const params = new URLSearchParams({
    ...
    lang: session.lang,
    id: myId,
    public: String(session.isPublic),
  });
  ...
  ws.onopen = () => {
    ...
    chat = new callMods!.chat.ChatManager({
      myLang: session!.lang,
      myId,
...
async function handleServer(msg: any): Promise<void> {
  switch (msg.type) {
    ...
    case "peer_joined": {
      ...
      if (msg.cartesia_voice_id)
        peerVoiceIds.set(msg.peer_id, msg.cartesia_voice_id); // spec 0108
    ...
    case "language_detected": {
      ...
      const badge = videoGrid.querySelector(
        `[data-peer="${cssEsc(msg.peer_id)}"] .peer-lang`,
      );
      if (badge)
        badge.textContent =
          `${FLAG[msg.lang] || ""} ${msg.lang.toUpperCase()}`.trim();
    ...
    case "translated_audio": {
      ...
      if (ttsOn && msg.speaker_id !== myId)
        pcmPlayback.enqueue(msg.speaker_id, msg.seq, msg.pcm16_b64);
      break;
    }
    ...
    case "engine_downgraded": {
      ...
      if (msg.peer_id === myId) {
        if (serverCaptureFormat !== null) {
          ...
          showNotif(
            t(
              msg.reason === "premium_at_capacity"
                ? "enginePremiumBusy"
                : "enginePremiumPaused",
            ),
          );
    ...
    case "subtitle_final": {
      ...
      if (msg.speaker_id === myId)
        callTimer.handleTranscript(msg.original || "");
      break;
    }
    ...
    case "balance_exhausted": {
      ...
      $("exhausted-title").textContent = t(
        loggedIn ? "outOfCredits" : "trialEnded",
      );
      $("exhausted-text").textContent = t(
        loggedIn ? "outOfCreditsText" : "trialEndedText",
      );
...
function addCell(
  ...
  if (!isSelf) {
    ...
    if (friendUid && friendUid !== auth.getUser()?.id)
      actions.appendChild(tileFriendButton(friendUid));
    if (billing && auth.isLoggedIn()) {
      ...
      reportBtn.addEventListener("click", () =>
        openReport(id, peerNames.get(id)?.name || name),
      );
...
function schedulePeerRemoval(id: string): void {
  ...
  videoGrid
    .querySelector(`[data-peer="${cssEsc(id)}"]`)
...
function updateGridCount(): void {
  videoGrid.dataset.peers = String(
    videoGrid.querySelectorAll(".video-cell").length,
  );
  layoutVideos();
}
...
function setupPan(cell: HTMLElement): void {
  ...
  let tx = 0,
    ...
    scale = 1;
  let startX = 0,
    startY = 0; // 1-finger pan anchor
  let startDist = 0,
    startScale = 1; // 2-finger pinch anchor
  ...
  cell.addEventListener(
    ...
    (e: TouchEvent) => {
      ...
      if (e.touches.length === 2 && startDist > 0) {
        scale = Math.min(
          4,
          Math.max(1, startScale * (dist(e.touches) / startDist)),
        );
...
function layoutVideos(): void {
  ...
  // Determine focus id
  const focusId =
    pinnedPeerId || (viewMode === "speaker" ? lastSpeakerId : null);
  const focusCell = focusId
    ? videoGrid.querySelector<HTMLElement>(`[data-peer="${cssEsc(focusId)}"]`)
    : null;
  ...
  if (focusCell && focusId && n > 1) {
    ...
    if (IS_MOBILE && focusCell.classList.contains("sharing"))
      setupPan(focusCell);
    ...
    if (lastSpeakerId && lastSpeakerId !== pinnedPeerId) {
      const as = videoGrid.querySelector<HTMLElement>(
        `[data-peer="${cssEsc(lastSpeakerId)}"]`,
      );
...
function spotlightShare(peerId: string, active: boolean): void {
  if (active) {
    ...
    const cell = videoGrid.querySelector<HTMLElement>(
      `[data-peer="${cssEsc(peerId)}"]`,
    );
    if (cell) {
...
function applyAudioMode(): void {
  ...
  videoGrid.querySelectorAll<HTMLElement>(".video-cell").forEach((cell) => {
    ...
    if (id === myId) {
      ...
      video.muted =
        !shareAudio && !!(ttsOn && peerLang && myLang && peerLang !== myLang);
    }
...
function setCameraOff(id: string, off: boolean): void {
  const cell = videoGrid.querySelector(`[data-peer="${cssEsc(id)}"]`);
  if (!cell) return;
  (cell.querySelector("video") as HTMLElement).style.visibility = off
    ? "hidden"
    : "visible";
...
function setAudioMuted(id: string, muted: boolean): void {
  const cell = videoGrid.querySelector(`[data-peer="${cssEsc(id)}"]`);
  if (cell)
    (cell.querySelector(".mute-indicator") as HTMLElement).hidden = !muted;
}
...
function setScreenShareIndicator(
  id: string,
  active: boolean,
  shareAudio = false,
): void {
...
function toggleInvite(force?: boolean): void {
  ...
  if (open) {
    inviteLinkInput.value = session
      ? buildInviteLink(location.origin, session.room)
      : "";
...
function updateInviteAvailability(count: number): void {
  ...
  if (!canInvite && !invitePanel.classList.contains("closed"))
    toggleInvite(false);
}
...
async function sendInvitesFromForm(): Promise<void> {
  ...
  if (emails.length > MAX_INVITE_EMAILS) {
    setInviteStatus(
      t("inviteTooMany").replace("{n}", String(MAX_INVITE_EMAILS)),
      "err",
    );
    return;
  }
  ...
  if (res.failed)
    msg += ` · ${t("inviteSomeFailed").replace("{n}", String(res.failed))}`;
...
sessionTimerEl.querySelector<HTMLElement>(".sb-ico")!.innerHTML = icon(
  "timer",
  13,
);
...
function renderHeaderTimes(): void {
  ...
  $("session-elapsed").textContent = formatClock(
...
function updateParticipantsList(): void {
  ...
  for (const [id, info] of peerNames) {
    items.push({
      ...
      lang: info.lang,
      isSelf: false,
      micMuted: peerMicMuted.get(id) ?? false,
      handRaised: peerHandRaised.get(id) ?? false,
  ...
  for (const p of items) {
    ...
    nameEl.textContent =
      `${FLAG[p.lang] || ""} ${p.name}${p.isSelf ? ` · ${t("you")}` : ""}`.trim();
...
function showSubtitle(
  speakerId: string,
  {
    ...
    original,
    interim,
  }: { translation?: string; original?: string; interim: boolean },
): void {
  ...
  if (
    !renderSubtitleInto(area, subtitleMode, { translation, original, interim })
  )
    return;
...
function setControlState(): void {
  ...
  btnFullscreen.innerHTML = icon(
    document.fullscreenElement ? "fullscreen-off" : "fullscreen",
  );
  ...
  setToggleState(
    btnShare,
    isSharingScreen,
    isSharingScreen ? t("stopShare") : t("screenShareTip"),
  );
  ...
  setToggleState(
    btnRecord,
    isRecording,
    isRecording ? t("recording") : t("recordingTip"),
  );
  ...
  btnMore.classList.toggle(
    "has-active",
    isSharingScreen || isRecording || handRaised,
  );
...
function setMoreOpen(open: boolean): void {
  ...
  if (open)
    moreMenu
      .querySelector<HTMLButtonElement>(".control-btn:not(.hidden)")
      ?.focus();
}
...
function currentRawCameraTrack(): MediaStreamTrack | null {
  ...
  return (
    localStream?.getVideoTracks().find((tr) => tr.readyState === "live") ?? null
  );
}
...
$("prejoin-audio-btn").addEventListener(
  "click",
  () => void openAudioSettings(),
);
$("btn-audio-settings").addEventListener(
  "click",
  () => void openAudioSettings(),
);
...
btnSubtitle.addEventListener("click", () => {
  ...
  document.querySelectorAll<HTMLElement>(".subtitle-area").forEach((a) => {
    a.innerHTML = "";
  });
  setControlState();
});
...
btnPip.addEventListener("click", () => {
  ...
  const pipSize =
    tileCount <= 1 ? { width: 320, height: 180 } : { width: 420, height: 320 };
    ...
    .then((w: Window) => {
      ...
      const carryScope = (from: Element | null, to: HTMLElement): void => {
        for (const a of from?.getAttributeNames() ?? [])
          if (a.startsWith("data-astro-cid"))
            to.setAttribute(a, from?.getAttribute(a) ?? "");
      };
...
function syncPip(): void {
  ...
  const live = new Set(
    [...videoGrid.querySelectorAll<HTMLElement>(".video-cell[data-peer]")].map(
      (c) => c.dataset.peer,
    ),
  );
  pipGrid
    .querySelectorAll<HTMLElement>(".video-cell[data-peer]")
    .forEach((c) => {
  ...
  videoGrid
    ...
    .forEach((liveCell) => {
      ...
      let cell = pipGrid.querySelector<HTMLElement>(
        `.video-cell[data-peer="${cssEsc(peer)}"]`,
      );
      if (!cell) {
        cell = liveCell.cloneNode(true) as HTMLElement;
        cell
          .querySelectorAll(".cell-actions, .pan-toggle, .pan-hint")
          .forEach((e) => e.remove());
  ...
  try {
    ...
    if (shareAudio.length) {
      try {
        ...
        shareAudioCtx
          .createMediaStreamSource(new MediaStream(shareAudio))
          .connect(dest);
        const mic = localStream?.getAudioTracks() ?? [];
        if (mic.length) {
          shareAudioCtx
            .createMediaStreamSource(new MediaStream(mic))
            .connect(dest);
        }
    ...
    screenPip.setCamera(
      camOn ? (localStream?.getVideoTracks()[0] ?? null) : null,
    );
...
function stopScreenShare(): void {
  ...
  ws?.send(
    JSON.stringify({ type: "screen_share", active: false, audio: false }),
  ); // tell peers (spec 0033)
...
function participantSource(
  ...
  return {
    ...
    name: isSelf
      ? session?.name || t("namePlaceholder")
      : peerNames.get(peerId)?.name || "Guest",
    stream,
...
function whiteboardRecordingSource(): ParticipantSource {
  return {
    peerId: WB_RECORDING_ID,
    name: t("whiteboardTip"),
    stream: whiteboard.captureStream(),
    videoOff: false,
  };
}
  ...
  if (blob.size > 0) {
    ...
    if (bizRecording && sessionId && billing && auth.isLoggedIn()) {
      ...
      if (ok) {
        ...
        auth.downloadBlob(
          blob,
          recordingFilename(session?.room || "call", new Date()),
        );
      }
    } else {
      auth.downloadBlob(
...
function fitChatInput(): void {
  ...
  const { height, overflowY } = resizeBox(
...
async function sendVoiceMessage(blob: Blob): Promise<void> {
  ...
  const ext = /mp4|m4a|aac/.test(blob.type)
    ? "m4a"
    : /ogg/.test(blob.type)
      ? "ogg"
      : "webm";
...
function overlayKeydown(e: KeyboardEvent): void {
  ...
  const focusables = Array.from(
    openOverlay.querySelectorAll<HTMLElement>(FOCUSABLE),
  ).filter(
...
function enterHome(): void {
  ...
  if (pendingInviteRoom) {
    ...
    onboarding.maybeAutoStartHome(
      () => openOverlay === null && !homeScreen.classList.contains("hidden"),
    );
  }
}
...
function updatePublicGate(): void {
  ...
  const pubBtn = visGroup.querySelector(
    '.seg-btn[data-vis="public"]',
...
async function loadWorkspaceNotes(): Promise<void> {
  ...
  renderWorkspaceNotes(
    orgId,
    projectId,
    await listProjectVoiceMessages(orgId, projectId),
  );
}
...
async function sendWsVoice(blob: Blob, durationS: number): Promise<void> {
  ...
  const res = await uploadProjectVoiceMessage(
    orgId,
    projectId,
    file,
    durationS,
  ...
  toast(
    res.translateBlocked === "credits"
      ? t("uploadNotTranslatedCredits")
      : t("wsVoiceSent"),
    res.translateBlocked === "credits" ? "err" : "ok",
  );
...
async function setupBizPrejoin(): Promise<void> {
  ...
  if (orgLabel)
    (orgLabel as HTMLElement).style.display = orgs.length > 1 ? "" : "none";
...
async function loadLedger(which: LedgerTab): Promise<void> {
  ...
  let rows: any[] =
    which === "history" ? await auth.fetchHistory() : await auth.fetchUsage();
  ...
  for (const s of sessions) {
    ...
    for (const format of ["pdf", "json"] as const) {
      ...
      btn.addEventListener("click", async () => {
        ...
        btn.disabled = false;
        if (!r.ok)
          toast(t(r.status === 429 ? "downloadRateLimited" : "downloadFailed"));
      });
...
async function downloadFromPostCall(
  format: "json" | "pdf",
  ...
  const r = await auth.downloadTranscript(
    postCallSessionId,
    format,
    getUiLang(),
  );
...
$("postcall-pdf").addEventListener(
  "click",
  (e) => void downloadFromPostCall("pdf", e.currentTarget as HTMLButtonElement),
);
$("postcall-json").addEventListener(
  "click",
  (e) =>
...
document.addEventListener("click", (e) => {
  if (
    ...
    !accountBar.contains(e.target as Node)
  )
    closeAccountMenu();
});
document.addEventListener("keydown", (e) => {
    ...
    els.incomingList.appendChild(
      friendRow(f, [
        friendActionBtn(
          ...
          true,
          false,
          () => void onFriendAccept(f.id),
        ),
        friendActionBtn(
          t("friendReject"),
          false,
          true,
          () => void onFriendRemove(f.id),
        ),
      ]),
    );
  }
...
async function addFriendByPeer(
  userId: string,
  btn: HTMLButtonElement,
): Promise<void> {
  btn.disabled = true;
...
function tileFriendButton(uid: string): HTMLButtonElement {
  ...
  if (friendIds.has(uid)) {
    ...
    action = async () => {
      ...
      toast(
        err === null ? t("friendRequestSent") : friendAddErrorText(err),
        err === null ? "ok" : "err",
      );
    };
  }
...
function accountFriendsVisible(): boolean {
  return (
    !accountScreen.classList.contains("hidden") &&
    currentAccountSection === "friends"
  );
}
...
function buildNotifMatrix(prefs: Prefs): void {
  const isOn = (type: string, ch: string) =>
    prefs.preferences.find((p) => p.type === type && p.channel === ch)
      ?.enabled ?? true;
...
function selectAccountSection(section: AccountSection): void {
  ...
  for (const tab of accountTabs)
    tab.setAttribute(
      "aria-selected",
      String(tab.dataset.acctSection === section),
    );
  ...
  switch (section) {
    ...
    case "privacy":
      ...
      $("voice-clone-state").textContent = localStorage.getItem(
        VOICE_CLONED_KEY,
      )
...
for (const tab of accountTabs)
  tab.addEventListener("click", () =>
    selectAccountSection(tab.dataset.acctSection as AccountSection),
  );
...
async function pollNotifications(): Promise<void> {
  ...
  const next = list.find(
    (n) =>
      (n.type === "friend_active" || n.type === "call_invite") &&
      !shownNotifIds.has(n.id),
  );
...
function startNotifPolling(): void {
  ...
  if (!friendNotifTimer)
    friendNotifTimer = window.setInterval(
      () => void pollNotifications(),
      20000,
    );
}
...
$("exhausted-dismiss").addEventListener("click", () =>
...
function syncConsentAccept(): void {
  const ok =
    $<HTMLInputElement>("consent-age").checked &&
    $<HTMLInputElement>("consent-tos").checked;
...
for (const el of document.querySelectorAll<HTMLElement>("[data-ico]"))
  el.innerHTML = icon(
    el.dataset.ico!,
    el.classList.contains("balance-ico") ? 15 : 18,
  );
...
const wbToolBtns = wbOverlay.querySelectorAll<HTMLButtonElement>(
  ".wb-tool[data-tool]",
);
wbToolBtns.forEach(
  (b) => (b.innerHTML = icon(WB_TOOL_ICON[b.dataset.tool as WbTool], 20)),
);
...
function setWbWidth(width: WbWidth): void {
  ...
  wbOverlay
    .querySelectorAll<HTMLButtonElement>(".wb-width")
    .forEach((b) => b.classList.toggle("active", b.dataset.width === width));
}
...
wbToolBtns.forEach((b) =>
  b.addEventListener("click", () => setWbTool(b.dataset.tool as WbTool)),
);
...
$("wb-page-del").addEventListener("click", () =>
  whiteboard.deleteCurrentPage(),
);
...
document.addEventListener("click", (e) => {
  if (
    !wbExportMenu.classList.contains("hidden") &&
    !wbExportWrap.contains(e.target as Node)
  )
    setWbExportOpen(false);
});
document.addEventListener("keydown", (e) => {
  if (e.key === "Escape" && !wbExportMenu.classList.contains("hidden"))
    setWbExportOpen(false);
});
...
function roomLangCodes(): string[] {
  return Array.from(
    new Set([
      session?.lang || "en",
      ...Array.from(peerNames.values()).map((p) => p.lang),
    ]),
  ).filter((l) => l && l !== "auto");
}
...
($("timer-badge").querySelector(".ti-ico") as HTMLElement).innerHTML = icon(
  "timer",
  15,
);
...
document.addEventListener("keydown", (e) => {
  if (e.key === "Escape" && !timerPop.classList.contains("hidden"))
    toggleTimerPop(false);
});
